...
class ForexEnv(TradingEnv):
    ...
    def _process_data(self):
        ...
        print(prices.shape)
        # check if it is uptrend or downtrend in the chosen window of 3
        prices_shift_wind = prices[3:]
        print("Shifted price", prices_shift_wind[:15])
        for i in range(3):  # filling with 0 for first three entries
            prices_shift_wind = np.insert(prices_shift_wind, -1, 0)
        print("Zeros appended", prices_shift_wind[:15])
        print("Prices List", prices[:15])
        trend = prices_shift_wind - prices  # Calculate difference of prices between lag of 3 days
    ...
    def _calculate_reward(self, action):
        ...
        if trade:
            ...
            if self._position == Positions.Short and self.trend < 0:
                ...
                step_reward -= 10000  # punish for taking wrong decision
    ...
    def _update_profit(self, action):
        ...
        if ((action == Actions.Buy.value and self._position == Positions.Short) or
                (action == Actions.Sell.value and self._position == Positions.Long)):
            trade = True  # validating that the action makes a trade
