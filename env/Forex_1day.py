...
class ForexEnv_1day(TradingEnv):

    def __init__(self, df, window_size, frame_bound, unit_side='left',trend_window=0):
        assert len(frame_bound) == 2
        assert unit_side.lower() in ['left', 'right']
        self.trend_window = trend_window
        self.frame_bound = frame_bound
        self.unit_side = unit_side.lower()
        super().__init__(df, window_size,trend_window)
    ...
    def _process_data(self):
        ...
        # print(prices.shape)
        # check if it is uptrend or downtrend in the chosen window of 15 days
        prices_shift_wind = prices[self.trend_window:]
        # print("Shifted price", prices_shift_wind[:15])
        # filling with 0 for in front of the array to match the length of prices array entries
    ...
    def _calculate_reward(self, action):
        ...
        if trade:
            ...
            if self._position == Positions.Short and self.trend < 0:
                ...
                step_reward -= abs(price_diff * 10000)  # punish for taking wrong decision+1 is to avoid div by 0
