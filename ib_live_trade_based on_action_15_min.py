...
def onConnected():
    print('Successfully connected with IB Gateway...... [{dateTime}].\n Printing account details'.format(
...
def onBarUpdate(bars, hasNewBar):
    ...
    if buy_count:
        print("Buy Count is > 0 ", "trade price is: ", trade_price, "latest_price: ", latest_price)
    ...
    if trade_date:
        ...
        if datetime.datetime.now() - trade_date < pd.Timedelta("P0DT0H5M0S"):  # To avoid error
            print("Less than 1 minute since last trade.")
            print("The time difference is:", datetime.datetime.now() - trade_date, latest_date - trade_date)
            return
    # if latest_date - reference_date < pd.Timedelta("P0DT0H15M0S") or df.shape[0] < 2 * window_for_trend_calc:
    if latest_date - reference_date > pd.Timedelta("P0DT0H15M0S") or first_time:
        print("Accumulated enough data or initiating a trading")
        ...
        # load the model
        model = A2C.load('./results_15_min_new/model_group_by_15_min_20000steps_5win')
        ...
        print("_" * 100)
        print("States After Prediction ", _states)
        print("_" * 100)
        ...
        order = do_trade(Actions(action), latest_price)  # If doing trade according to action
        # order = do_trade(env._position, latest_price)  # If doing trade according to position
        print("Order details: ", order)
        trade = ib.placeOrder(contract, order)
        trade_date = datetime.datetime.now()
        print("Trade Date Setting after calling do trade: ", trade_date)
        print("Trade", trade)

    else:
        print("Inserted less than 15 mins ago or still accumulating data")
...
def do_trade(action, price):
    ...
    global first_time
    order = None
    print("Action: ", action, "and Sell Count: ", sell_count, "and Buy count: ", buy_count)
    if action == Actions.Sell and sell_count == 0:  # allowing only one trade to happen at a trading session
    ...
    elif action == Actions.Buy and buy_count == 0:  # allowing only one trade to happen at a trading session
...
bars = ib.reqHistoricalData(
    ...
    barSizeSetting='15 mins', whatToShow='MIDPOINT', useRTH=True, keepUpToDate=True)
df = util.df(bars)
print("*" * 50)
